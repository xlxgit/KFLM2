...
data_name='Lipo_opt'
model_name='CombinedAttModel'
dropout = 0.1
hidden_dim = 16

# 数据集和数据加载器
csv_file = '../Lipophilicity.csv'  # 替换为您的 CSV 文件路径
data = pd.read_csv(csv_file)
smiles = data['smiles'].tolist()
logP = data['exp'].tolist()
...
model_save_path = f'result/{data_name}_{model_name}_early.pt'
...
for idx in range(10):
    ...
    combined_model = CombinedAttModel(embedding_dim, hidden_dim=16, dropout=0.1).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if abs(best_epoch - epoch) >= 20:
            print("=" * 20 + f' early stop at epoch {epoch+1} ' + "=" * 20)
            break
