...
for idx in range(10):
    ...
    # 初始化联合模型
    input_dim = 1536
    embed_model = EmbeddingModel(input_dim, hidden_dim=hidden_dim, dropout=dropout).to(device)
    
    # 损失函数和优化器
    criterion = nn.MSELoss()
    optimizer = torch.optim.Adam(embed_model.parameters(), lr=0.001)
    
    # 训练模型
    num_epochs = 100
    for epoch in range(num_epochs):
        embed_model.train()  # 设置模型为训练模式
        ...
        for (smiles_embeddings, graph_data), logP_values in train_loader:
            ...
            # 前向传播
            predictions = embed_model(smiles_embeddings)
